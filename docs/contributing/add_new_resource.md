...
By default, the Terraform Validator tests will compare the expected `example_product_resource.json` result with the actual result from the test environment, and fail if they do not match. If you need more control over how matching is done (eg. check that an array contains a value), you can use a custom compare function, and configure the test in [`cli_test.go.erb`](https://github.com/GoogleCloudPlatform/magic-modules/blob/18701ee8cd46aa488a84b36ebfe567e592933b9d/mmv1/third_party/validator/tests/source/cli_test.go.erb#L67-L73) and [`read_test.go.erb`](https://github.com/GoogleCloudPlatform/magic-modules/blob/18701ee8cd46aa488a84b36ebfe567e592933b9d/mmv1/third_party/validator/tests/source/read_test.go.erb#L26-L32). This is currently being done for `*_iam_member` and `*_iam_binding` resources in order to ignore preexisting premissions in the test environment.
